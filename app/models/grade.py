...
class GradeModel:
    ...
    def add_grade(self, teacher_id, student_id, subject_id, grade, comment=""):
        query = "INSERT INTO grades (student_id, teacher_id, subject_id, grade, comment) VALUES (%s, %s, %s, %s, %s)"
        self.db.execute(query, (student_id, teacher_id, subject_id, grade, comment))
    ...
    def get_student_grade_by_student(self, student_id):
        query = """
        SELECT g.id, g.grade, g.comment, g.created_at AS date_added, sub.name AS subject_name, t.first_name AS teacher_name
        FROM grades g
        JOIN subjects sub ON g.subject_id = sub.id
    ...
    def get_student_grades_by_subject(self, student_id, subject_id):
        query = """
        SELECT g.grade, g.comment, g.created_at AS date_added, sub.name AS subject_name, t.first_name AS teacher_name
        FROM grades g
        JOIN subjects sub ON g.subject_id = sub.id
        JOIN teachers t ON g.teacher_id = t.id
