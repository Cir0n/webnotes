...
class AdminViews:
    ...
    def register_routes(self):
        ...
        @self.admin_bp.route("/edit_student/<student_id>", methods=["GET", "POST"])
        def edit_student(student_id):
            ...
            if request.method == "POST":
                ...
                return redirect(url_for("admin_bp.list_students"))
            return render_template("admin/edit_student.html", student=result, classes=classes, languages=languages, options=options)
        ...
        @self.admin_bp.route("/edit_teacher/<teacher_id>", methods=["GET", "POST"])
        def edit_teacher(teacher_id):
            ...
            if request.method == "POST":
                ...
                result = self.teacher_controller.edit_teacher(
                    teacher_id, first_name, last_name, class_id, selected_languages, selected_options
                )
                flash("Enseignant modifié avec succès")
                return redirect(url_for("admin_bp.list_teachers"))
            return render_template("admin/edit_teacher.html", teacher=result, classes=classes, languages=languages, options=options)
