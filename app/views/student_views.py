...
class StudentViews:
    ...
    def register_routes(self):
        ...
        def student_dashboard():
            ...
            if not student or isinstance(student, str) or "error" in student:
            ...
            if hasattr(student, 'get'):
                ...
                if class_id:
                    class_student = self.classController.get_one_class(class_id)
                    if not isinstance(class_student, list):
                        class_student = [class_student] if class_student else []
        ...
        def student_schedule():
            student_id = session.get("user_id")
            student = self.studentController.get_student_info(student_id)
            class_student = self.classController.get_one_class(student["class_id"]) if student.get("class_id") else None
            return render_template(
